...
#' @param quarter Calendar quarter for most recent Naomi estimates. This is T2
#' for all countries with the exception of MWI and ZAF who have additional
#' historical survey projections using T3.
#'
#'
...
shipp_format_naomi <- function(outputs, options, quarter){
...
#' @param naomi_pop Naomi population estimates for current time point.
...
shipp_disaggregate_fsw <- function(outputs,
  ...
  afs <- naomi.resources::load_shipp_exdata("afs", iso)

  # Select birth cohort from 2000, to turn 15 in 2015
  cohort <- 2000

  afs <- afs %>%
    dplyr::filter(yob == cohort, sex == "female", iso3 == iso) %>%
                                   ...
                                   kp_wb)
{
  ...
  if(consensus_est == "kp_wb") {
    ...
    pwid_consensus <- kp_wb[kp_wb$key_population == "PWID", ]$population_size
                                  ...
                                  )
{
  ...
  # Scale to consensus estimate of MSM from KP wokrbook if specified

  # Default: Scale estimates to national PSE from Goals
  if(consensus_est == "goals") {
    msm_consensus <- goals$`msm-pse`
  }
  ...
  if(consensus_est == "kp_wb") {
    ...
    msm_consensus <- kp_wb[kp_wb$key_population == "MSM", ]$population_size
  ...
  afs <- afs %>%
    dplyr::filter(yob == cohort, sex == "male", iso3 == iso3) %>%
...
#' @param survey_year_sample Year of survey to sample estimates.
#' @param quarter Calendar quarter for most recent Naomi estimates. This is T2
#' for all countries with the exception of MWI and ZAF who have additional
#' historical survey projections using T3.
#'
...
shipp_calculate_prevalence_female <- function(naomi_output,
  ...
  naomi_est <- naomi_output %>%
    dplyr::filter(calendar_quarter == quarter,
                  sex == "female",
...
#' Prep women's outputs for spreadsheet tabs
#'
...
shipp_generate_risk_populations <- function(naomi_output,
  ...
  # Format naomi output
  naomi <- shipp_format_naomi(outputs, options, quarter)
  ...
  female_logit_prevalence <- shipp_calculate_prevalence_female(naomi$naomi_long,
                                                              ...
                                                              fsw_est,
                                                              female_srb,
                                                              survey_year_sample = survey_year,
                                                              quarter)
...
#'
#' This doesn't write column headers into the workbook, it expects
...
generate_shipp_tool <- function(output, pjnz, path = tempfile(fileext = ".xlsx"),
  ...
  write_xlsx_sheets(template_path, sheets, path)
...
extract_kp_workbook <- function(pjnz_list){
  ...
  pjnz_list <- naomi:::unroll_pjnz(pjnz_list)

  # Extract .DP files
  dp <- lapply(pjnz_list, naomi:::read_dp)
