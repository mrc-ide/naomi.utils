...
calc_survey_indicators <- function(survey_meta,
  ...
  ## keep only variables we want depending on our outcomes
  if(hiv_outcomes) {
  ...
  ## Include res_type stratified and all if res_type is TRUE, otherwise recode to all
  if(by_res_type)
  ...
  ## Data prep for biomarkers indiciators only
  ...
  cnt <- dat %>%
    dplyr::group_by(indicator, survey_id, area_id, res_type, hiv_type, sex, age_group) %>%
  ...
  datspl <- dat %>%
    dplyr::mutate(spl = paste(indicator, survey_id, area_level, res_type, hiv_type, sex, age_group)) %>%
    split(.$spl)
  ...
  do_svymean <- function(df) {
    ...
    val <- survey::svyby(~estimate,
                         ~ indicator + survey_id + area_id + res_type + hiv_type + sex + age_group,
                         des, survey::svymean)
  ...
  val <- cnt %>%
    dplyr::full_join(
             ...
             by = c("indicator", "survey_id", "area_id", "res_type", "hiv_type", "sex", "age_group")
           ) %>%
    dplyr::left_join(
  ...
  ## Remove hiv_type if looking at biomarker outcomes because it is confusing
  if(hiv_outcomes) {
    val <- val %>% dplyr::select(-hiv_type)
  }
